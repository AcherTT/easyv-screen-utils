...
class ScreenImporterAsyncWorker : public AsyncWorker {
 ...
 private:  // 映射关系
  ...
  std::map<std::string, std::string>
      reservePlanCollectionIdMap_;             // 预案集合数据
  std::map<uint64_t, uint64_t> containerMap_;  // 容器数据
  std::map<uint64_t, rapidjson::Document>
      needImportContainersConfigMap_;                     // 容器组件数据
 ...
 private:                                         // 解析出来的数据
  ...
  rapidjson::Document *componentContainerRelData_ =
      nullptr;  // 数据容器组件关系配置文件内容
  rapidjson::Document *componentCustomInfoData_ =
      nullptr;  // 自定义组件配置文件内容
  ...
  rapidjson::Document *reservePlanCollectionData_ =
      nullptr;                                 // 预案集合配置文件内容
 ...
 private:                   // 工具函数
  ...
  rapidjson::Document *getFileContent(
      std::string path);  // 获取包文件内容并解析
  rapidjson::Document *getFileContent(
 ...
 public:
  ScreenImporterAsyncWorker(Function &callback, std::string packetPath,
